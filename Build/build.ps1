...
$Author = 'Kevin Lee'
$PowerShellForWebInspect = 'PowerShellForWebInspect'
...
elseif ($env:APPVEYOR_PULL_REQUEST_NUMBER -gt 0) {
    ...
    Try {
        ...
        $manifestPath = ".\$PowerShellForWebInspect\$PowerShellForWebInspect.psd1"
        ...
        $functionList = ((Get-ChildItem -Path .\$PowerShellForWebInspect\Public).BaseName)
        $splat = @{
            'Path'              = $manifestPath
        ...
        (Get-Content -Path $manifestPath) -replace "PSGet_$PowerShellForWebInspect", "$PowerShellForWebInspect" | Set-Content -Path $manifestPath
        (Get-Content -Path $manifestPath) -replace 'NewManifest', $PowerShellForWebInspect | Set-Content -Path $manifestPath
    ...
    Import-Module -Name "$PSScriptRoot\..\$PowerShellForWebInspect" -Force
    New-MarkdownHelp -Module $PowerShellForWebInspect -OutputFolder '..\docs\' -Force
    New-ExternalHelp -Path '..\docs\' -OutputPath "..\$PowerShellForWebInspect\en-US\" -Force
    . .\Build\docs.ps1
    Write-Host -Object ''
    ...
    if ($SkipPublishing) {
        ...
        Try {
            ...
            Write-Host "$PowerShellForWebInspect PowerShell Module version $newVersion published to GitHub." -ForegroundColor Cyan
        }
        Catch {
            # Sad panda; it broke
